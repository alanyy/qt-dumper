...
def qform___SASSTL__map():
    return mapForms()

def qdump___SASSTL__map(d, value):
    impl = value["_M_t"]
    size = int(impl["_M_node_count"])
    check(0 <= size and size <= 100*1000*1000)
    d.putItemCount(size)
...
def qform__sup__String(d, value):
    return "Inline,In Separate Window"

def qdump__sup__String(d, value):
    member = value['m_tState']['member']
    d.putNumChild(0)
    ta0 = d.templateArgument(value.type, 0)
    charSize = ta0.sizeof
    if charSize == 1:
        ...
        displayType = DisplayUtf16String
    memPtr = member.cast(lookupType("void").pointer())
    if isNull(memPtr):
        d.putValue("", encodingType)
    start  = value['m_tState']['member']['m_pStart']
    finish = value['m_tState']['member']['m_pFinish']
    startPtr = start.cast(ta0.pointer())
    finishPtr = finish.cast(ta0.pointer())
    if pointerValue(startPtr) == pointerValue(finishPtr):
        d.putValue("", encodingType)
    size = (finishPtr - startPtr) * charSize
    cutoff = min(size, qqStringCutOff)
    mem = d.readRawMemory(startPtr, cutoff)
    ...
    if format == 1:
        ...
        d.putField("editvalue", d.readRawMemory(startPtr, size))
...
def qdump___SASSTL__vector(d, value):
    type = d.templateArgument(value.type, 0)
    alloc = value["_M_end_of_storage"]["_M_data"]
    isBool = type.code == gdb.TYPE_CODE_BOOL
    if isBool:
        start = value["_M_start"]["_M_p"]
        finish = value["_M_finish"]["_M_p"]
        # FIXME: 8 is CHAR_BIT
        storage = d.lookupType("unsigned long")
        storagesize = storage.sizeof * 8
        size = (finish - start) * storagesize
        size += value["_M_finish"]["_M_offset"]
        size -= value["_M_start"]["_M_offset"]
    else:
